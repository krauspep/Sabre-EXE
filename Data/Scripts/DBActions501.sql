CREATE OR ALTER PROCEDURE GET_DATE_RANGE ( start_date DATE, end_date DATE )
RETURNS ( DayNO INT, TheDate DATE, DAY_OF_THE_WEEK VARCHAR(3) )
AS
  DECLARE i             INT  = 0;
  DECLARE num_days      INT  = 0;
BEGIN
  num_days = end_date - start_date;
  WHILE ( i <= num_days ) DO
  BEGIN
    DayNo = i;
    TheDate = start_date + i;
    DAY_OF_THE_WEEK = CASE EXTRACT( WEEKDAY FROM TheDate ) 
                        WHEN 0 THEN 'SUN' 
                        WHEN 1 THEN 'MON' 
                        WHEN 2 THEN 'TUE'
                        WHEN 3 THEN 'WED'
...
CREATE OR ALTER PROCEDURE CREATE_DUTY_SCHEDULE ( AEmployeeKey INT, AStartDate DATE, AEndDate DATE )
AS
    ...
    SELECT WS.WORK_SCHEDULE_KEY,
      ...
      FROM EMPLOYEE E 
           ...
           JOIN SHIFT_ASSIGN_WEEK SAW ON SAW.SHIFT_KEY = S.SHIFT_KEY AND 
                                         SAW.ASSIGNED_WEEK_NO = (EXTRACT(YEARDAY FROM THEDATE) - EXTRACT(WEEKDAY FROM THEDATE-1) + 7) / 7 AND 
